...
> 🚧 项目中（Work in Progress）— 后端管道已完成，前端审核界面开发中。

AI 辅助照片清理工具 — 扫描你的照片目录，自动去重、质量评分、按视觉相似度和日期聚类分组，生成结构化审核报告。你只需要在审核界面里点"保留 / 归档"，不需要手动一张张翻几万张照片。

**当前阶段**: Phase A 完成。后端七阶段管道（EXIF 扫描 → dHash-SimHash 去重 → 质量评分 → CLIP+HDBSCAN 聚类 → 日期分组 → JSON 报告）跑通，可用合成图端到端验证。Phase B 前端审核界面正在开发。

## 功能一览

- **只读处理** — 不修改、不复制源文件，唯一输出是 JSON 报告
- **双哈希去重** — dHash 汉明距离 ≤ 5 ∪ SimHash 余弦相似度 ≥ 0.95，覆盖截图、压缩、调色等常见变换
- **质量评分** — 清晰度 + 曝光 + 尺寸五维加权，每张照片输出 0–100 分及理由
- **视觉聚类** — CLIP ViT-B/32 嵌入 + HDBSCAN，自动把相似照片分到一组
- **日期分组** — 按天/周/月/年桶化，与视觉簇做笛卡尔积得到最终分组
- **审核报告** — 结构化 JSON，可直接喂给前端审核界面交互操作
